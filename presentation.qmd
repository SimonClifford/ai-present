...
## The prototype
::: {.notes}
1. Inherited the project from an earlier incarnation developed by Piero and Matt.
2. The code has various scripts that fire on particular XNAT triggers to perform the
algorithm.
...
1. Obviously requiring root permissions to start is a problem on HPC.  Processes
run as root on the host, but are confined by the container system.  If a user
can _start_ a container on the system they can likely break out of the
container.
1. This is how Docker does it, other systems are different (e.g. Singularity).
:::

::: {.incremental}
- Docker: platform for building, shipping, and running applications.
- _Containers_: more lightweight than virtualisation.
- Requires **root** privileges to manage.
:::
...
## Medical Images
::: notes
1. Prototype was handed images directly.
1. We need to get them while in a hospital setting.
...
::: {.notes}
1. Can run in Windows, using WSL.
:::
::: {.incremental}
- Adds or removes complexity, depending on POV.
- Adds potential for mother of all supply side attacks.
  - But we can choose when to update containers.
  - And we can audit them.
- Fairly good OS independence.
:::

## Thanks
This collection of containers runs on a VM in Arcus, for which I am very
grateful.
